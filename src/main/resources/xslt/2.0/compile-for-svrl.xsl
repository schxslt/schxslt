<xsl:transform version="2.0"
  ...
  <!-- Input: any Schematron file -->
  <!-- Output: an xslt file -->
  <!-- The output xslt file is intended to be used to take an input XML file and generate an svrl validation report equivalent to 
    the Schematron report generated by oXygen or other validation environments -->
  
  <!-- The following import statements handle pre-processing of the input -->
  <xsl:import href="include.xsl"/>
  <xsl:import href="expand.xsl"/>
  
  <!-- For any input schematron file, master/initial template is in the following import, for root element sch:schema -->
  <!-- This file is also the hub for three other include files and one imported file -->
  <xsl:import href="compile/compile-2.0.xsl"/>

  <!-- Has the input already been through include.xsl? -->
  <xsl:param name="input-includes-already-resolved" as="xs:boolean" select="false()"/>
  <!-- Has the input already been through expand.xsl? -->
  <xsl:param name="input-already-expanded" as="xs:boolean" select="false()"/>
  
  <xsl:variable name="input-included" as="document-node()">
    <xsl:choose>
      <xsl:when test="$input-includes-already-resolved">
        <xsl:sequence select="/"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:apply-templates select="/" mode="schxslt:start-include"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:variable>
  
  <xsl:variable name="input-expanded" as="document-node()">
    <xsl:choose>
      <xsl:when test="$input-already-expanded">
        <xsl:sequence select="/"/>
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsldoc:doc xmlns:xsldoc="http://www.oxygenxml.com/ns/doc/xsl">
    <xsldoc:desc>
      <xsldoc:p>Entry point for input Schematron file, inclusions resolved and expansions
        performed.</xsldoc:p>
    </xsldoc:desc>
  </xsldoc:doc>
  <xsl:template match="/" mode="abc">
    <xsl:apply-templates select="$input-expanded"/>
  </xsl:template>
